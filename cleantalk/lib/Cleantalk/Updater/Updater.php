...
class Updater {
    ...
    public function getLatestRelease()
    {
        try {
            ...
            if ( !$url ) {
                throw new Exception('cannot create latest GitHub release URI.');
            }

            $github_response = Helper::http__request($url, array(), 'github_api');

            if ( is_array($github_response) ) {
                throw new Exception(!empty($github_response['error'])
            ...
            throw new Exception('no correct data found.');
        } catch ( \Exception $e ) {
            return array('error' => 'Updater: Error on getting Github API response: ' . $e);
        }
