...
class Storage extends \ArrayObject {
	 ...
	 */
	public $storage_name;
	public $directory = CLEANTALK_ROOT;
	private $type = 'php';
	private $map;
	...
	/**
	 * Converts array to \Cleantalk\ApbctUni\Storage type.
	 * Recursive.
	 *
	 * @param $data_name
	 * @param array $data
	...
	public function convertToStorage( $data_name, $data ){
		...
		return new \Cleantalk\ApbctUni\Storage( $data_name, $data );
	}

	/**
	 * Converts \Cleantalk\ApbctUni\Storage to array type.
	 * Recursive.
	 *
	 * @param null $data
	 *
	...
	public function convertToArray( $data = null ){
		...
		$tmp = array();
		foreach ( $data as $name => $item ) {
			$tmp[ $name ] = $item instanceof \Cleantalk\ApbctUni\Storage
