...
use Cleantalk\ApbctUni\Storage;

class FileDB
{
    const FS_PATH = CLEANTALK_ROOT . 'data' . DS;
    
    /**
     * @var \Cleantalk\ApbctUni\File\Storage
     */
    private $storage;
    ...
    public function __construct($db_name)
    {
        ...
        if (!$this->meta->is_empty()) {
            $this->storage = new \Cleantalk\ApbctUni\File\Storage($db_name, $this->meta->cols);
            if ($this->meta->indexes) {
                $this->getIndexes();
            }
        }

        $this->getMetaDataTemp();
        if (! $this->meta_temp->is_empty()) {
            $this->storage = new \Cleantalk\ApbctUni\File\Storage( $db_name, $this->meta_temp->cols );
